...
{
    public sealed class SimpleTextMesh : IDrawable, IDisposable
    {
        ...
        VerticalDirection vdir = VerticalDirection.Down;
        ...
        public bool StaticCharWidth
        {
            get => use_static_char_width; set
            {
        ...
        private void recreate_mesh()
        {
            ...
            foreach (var c in text.Pattern("\n", "\r\n").Replace(C => C == '\r', '\n'))
            {
                if (c == '\n')
                {
                    ...
                    else if (hdir == HorizontalDirection.Center)
                    {
                        offset -= f.Spaceing.Horizontal;
                        offset /= 2;
                        foreach (var kvp in mesh_parts)
            ...
            else if (vdir == VerticalDirection.Center)
            {
        ...
        {
            Matrix4 scl = Matrix4.CreateScale(size);
            var f = GetFont();
            for (int i = 0; i < meshes.Length; i++)
            {
                var m = meshes[i];

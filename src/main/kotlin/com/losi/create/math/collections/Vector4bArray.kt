...
{
    companion object {
        private const val MASK: Byte = 0b00001111
        private val _empty = Vector4bArray(ByteArray(0))
        val empty: Vector4bArray get() = _empty

        private fun ByteArray.isAt(index: Int, value: Vector4b) = (this[index] and MASK) == (value.composite and MASK)
        ...
        @JvmExposeBoxed @JvmStatic fun of(count: Int): Vector4bArray = Vector4bArray(count)
        @JvmExposeBoxed @JvmStatic fun of(vararg elements: Vector4b): Vector4bArray {
            val arr = Vector4bArray(elements.size)
    ...
    constructor(vararg elements: Vector4b): this(ByteArray(elements.size))
    { elements.forEachIndexed { index, it -> this[index] = it } }

    operator fun get(index: Int): Vector4b {
        checkIndex(index)
        return Vector4b(elements[index])
    }
    operator fun set(index: Int, value: Vector4b) {
        checkIndex(index)
    ...
    data class Iterator internal constructor(private val array: ByteArray, private var cursor: Int = 0) : kotlin.collections.Iterator<Vector4b> {
        override fun next(): Vector4b = if (hasNext()) Vector4b(array[cursor++]) else throw NoSuchElementException()
    ...
    fun asSequence(): Sequence<Vector4b> {
    ...
    @JvmInline value class List internal constructor(private val array: Vector4bArray):  kotlin.collections.List<Vector4b> {
        override fun contains(element: Vector4b) = array.contains(element)
        override fun containsAll(elements: Collection<Vector4b>) = array.containsAll(elements)
        override fun get(index: Int) = array[index]
        override fun indexOf(element: Vector4b) = array.indexOf(element)
        override fun isEmpty() = array.size == 0
        override fun iterator() = array.iterator()
        override fun lastIndexOf(element: Vector4b) = array.lastIndexOf(element)
    ...
    @ConsistentCopyVisibility
    data class SpanList internal constructor(private val array: Vector4bArray, private val start: Int, private val count: Int): kotlin.collections.List<Vector4b> {
        override fun contains(element: Vector4b): Boolean = indexOf(element) >= 0
        override fun containsAll(elements: Collection<Vector4b>) = array.containsAll(elements, start, start + count)
        override fun get(index: Int): Vector4b { checkIndex(index); return array[index + start] }
        override fun toString(): String = array.toString()
        override fun indexOf(element: Vector4b): Int {
        ...
        override fun iterator(): kotlin.collections.Iterator<Vector4b> = iterator {
            for (i in start until start + count)
                yield(array[i])
        }
        override fun lastIndexOf(element: Vector4b): Int {
        ...
        @ConsistentCopyVisibility
        data class ListIterator internal constructor(private val array: Vector4bArray, private val start: Int, private val count: Int, private var index: Int = 0):
                kotlin.collections.ListIterator<Vector4b> {
    ...
    @ConsistentCopyVisibility
    data class ListIterator internal constructor(private val array: ByteArray, private var index: Int = 0): kotlin.collections.ListIterator<Vector4b> {
        fun get(index: Int) = Vector4b(array[index])
        override fun next() = get(index++)
        override fun previous() = get(index--)
    ...
    fun copyOf() = Vector4bArray(elements.copyOf())
    fun copyOf(newSize: Int): Vector4bArray {
        val result = Vector4bArray(newSize)
        for (i in 0 until minOf(size, newSize)) result.elements[i] = this.elements[i]
        return result
    }

    fun fill(value: Vector4b, fromIndex: Int = 0, toIndex: Int = size) {
        require(fromIndex in 0..size && toIndex in fromIndex..size)
        for (i in fromIndex until toIndex) this.elements[i] = value.composite
    }
    fun filled(value: Vector4b, fromIndex: Int, toIndex: Int): Vector4bArray {
        val copy = copyOf()
    ...
    fun reversed(): Vector4bArray {
        val copy = copyOf()
        copy.reverse()
        return copy
    }

    fun indexOf(element: Vector4b): Int {
        for (i in 0 until size) if (elements.isAt(i, element)) return i
        return -1
    }
    fun lastIndexOf(element: Vector4b): Int {
        for (i in size - 1 downTo 0) if (elements.isAt(i, element)) return i
        return -1
    }

    fun contains(element: Vector4b): Boolean = indexOf(element) >= 0
    fun containsAll(elements: Collection<Vector4b>, fromIndex: Int = 0, toIndex: Int = size): Boolean {
    ...
    inline fun forEach(action: (Vector4b) -> Unit) {
        for (i in 0 until size) action(this[i])
    }
    inline fun forEachIndexed(action: (index: Int, value: Vector4b) -> Unit) {
